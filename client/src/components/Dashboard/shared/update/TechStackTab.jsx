...
export default function TechStackTab({techStack, setTechStack}) {
  ...
  return (
    <div className="container mx-auto p-4 space-y-8">
      ...
      {categories.map(({ id, label }) => (
        <div
          key={id}
          className={`space-y-4 ${enabledCategories[id] ? "" : "opacity-50"}`}
        >
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor={`enable-${id}`} className="text-xl font-semibold">
              {label}
            </Label>
          </div>
